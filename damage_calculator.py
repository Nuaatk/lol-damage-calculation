class DamageCalculator:
    @staticmethod
    def calculate_team_damage(team_formation, players_damage):
        """计算每个队伍的伤害总和"""
        ...
        for player in team_formation['team1']:
            if player['summoner_name'] in damage_dict:
                team_damage['team1'] += damage_dict[player['summoner_name']]
        
        # 计算team2伤害（3楼乘以2）
        for player in team_formation['team2']:
            if player['summoner_name'] in damage_dict:
                team_damage['team2'] += damage_dict[player['summoner_name']] * 2
        
        # 计算team3伤害
        for player in team_formation['team3']:
            if player['summoner_name'] in damage_dict:
                team_damage['team3'] += damage_dict[player['summoner_name']]
